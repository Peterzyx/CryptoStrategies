...
def decay_linear(df, period=10):
    ...
    na_series = df.to_numpy() # df.as_matrix() is deprecated
...
class Alphas(object):
    ...
    def alpha001(self):
        inner = self.close.copy() # copy to avoid self-mutation
...
#         return alpha
    def alpha021(self):
        cond_1 = sma(self.close, 8) + stddev(self.close, 8) < sma(self.close, 2)
        cond_2 = sma(self.close, 8) - stddev(self.close, 8) < sma(self.close, 2)
    ...
    def alpha027(self):
        df = rank(sma(correlation(rank(self.volume), rank(self.vwap), 6), 2))
        alpha = pd.DataFrame(np.ones_like(self.close), index=self.close.index)
        alpha[df > 0.5] = -1
        return alpha
    ...
    def alpha030(self):
        ...
        delay2 = delay(self.close, 2)
        delay3 = delay(self.close, 3)
        inner = sign(self.close - delay1) + sign(delay1 - delay2) + sign(delay2 - delay3)
    ...
    def alpha032(self):
        return scale((sma(self.close, 7) - self.close)) + (20 * scale(correlation(self.vwap, delay(self.close, 5), 1440))) # 1440 for 24h 
    ...
    def alpha034(self):
        p1 = 1 - rank(stddev(self.returns, 2) / stddev(self.returns, 5))
        p2 = 1 - rank(delta(self.close, 1))
        return rank(p1 + p2)
    ...
    def alpha036(self):
        adv20 = sma(self.volume, 20)
        return (((((2.21 * rank(correlation((self.close - self.open), delay(self.volume, 1), 15))) + (0.7 * rank((self.open - self.close)))) + (0.73 * rank(ts_rank(delay((-1 * self.returns), 6), 5)))) + rank(abs(correlation(self.vwap, adv20, 6)))) + (0.6 * rank(((sma(self.close, 14400) - self.open) * (self.close - self.open)))))
    
    # Alpha#37	 (rank(correlation(delay((open - close), 1), close, 200)) + rank((open - close)))
    def alpha037(self):
        return rank(correlation(delay(self.open - self.close, 1), self.close, 1440)) + rank(self.open - self.close)
    ...
    def alpha045(self):
        df1 = sma(delay(self.close, 5), 20)
        df2 = correlation(self.close, self.volume, 2).fillna(value=0) # there are two consecutive close unchanged so corr = na
        df3 = correlation(ts_sum(self.close, 5), ts_sum(self.close, 20), 2).fillna(value=0)
        return -1 * (rank(df1) * df2 * rank(df3))
    ...
    def alpha052(self):
        df1 = ts_min(self.low, 5)
        return (((-1 * df1 + delay(df1, 5)) * 
                 rank(((ts_sum(self.returns, 1440) - ts_sum(self.returns, 60)) / 1440))) * ts_rank(self.volume, 5))
...
#        return (0 - (1 * (rank((sma(self.returns, 10) / sma(sma(self.returns, 2), 3))) * rank((self.returns * self.cap)))))
    ...
    def alpha064(self):
        ...
        return ((rank(correlation(ts_sum(((self.open * 0.178404) + (self.low * (1 - 0.178404))), 13), ts_sum(adv120, 13), 17)) < rank(delta(((((self.high + self.low) / 2) * 0.178404) + (self.vwap * (1 -0.178404))), 3.69741))) * -1)
    ...
    def alpha066(self):
        return ((rank(decay_linear(delta(self.vwap, 4).to_frame(), 7).CLOSE) + ts_rank(decay_linear(((((self.low* 0.96633) + (self.low * (1 - 0.96633))) - self.vwap) / (self.open - ((self.high + self.low) / 2) + self.epsilon)).to_frame(), 11).CLOSE, 7)) * -1)
    ...
    def alpha071(self):
        ...
        p2=ts_rank(decay_linear((rank(((self.low + self.open) - (self.vwap * 2))).pow(2)).to_frame(), 16).CLOSE, 4)
    ...
    def alpha083(self):
        return (rank(delay((self.high - self.low) / sma(self.close, 5), 2) * rank(rank(self.volume))) / (((self.high - self.low + self.epsilon) / sma(self.close, 5)) / (self.vwap - self.close + self.epsilon)))
    ...
    def alpha101(self):
        return (self.close - self.open) /(self.high - self.low + self.epsilon)
